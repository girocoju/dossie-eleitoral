...
| T-429 CPF no campo do nome do doador (ADR-051) | OK | 06/09/2026: a carga parou no `dbt build` com `assert_cpf_de_doador_nunca_em_claro` falhando. Uma linha em 82.710 do arquivo novo de prestacao de contas trazia CPF VALIDO no campo `nome_doador` — nao e' a coluna de CPF vazando, e' quem preencheu que digitou no lugar errado. Nada foi exposto: a etapa e' fatal e interrompeu antes do `gerar_site`; o `doadores.json` no ar tinha zero campos com 11 digitos. Nome com forma de CPF passa a virar NULL no staging, com `nome_era_cpf` marcando. A correcao quase quebrou o mart: `array_agg` do BigQuery lanca erro com elemento nulo — resolvido com `ignore nulls` |
| T-428 Identidade partida na fronteira do CPF (ADR-050 / fecha L-31) | OK | 05/09/2026: a L-31 dizia HOMONIMIA e propunha desempatar por UF; as duas identidades de Zequinha Marinho sao ambas do PA, e a segunda e' byte a byte a propria `chave_nome_nascimento`. `id_pessoa` cai no fallback quando o ano nao publica CPF — 96,9% em 1998 —, e a mesma pessoa fica com dois ids ao cruzar a fronteira. Das 326 chaves com mais de um id, 210 sao fronteira (204 sem nenhum ano em comum) e 116 homonimia real. A ambiguidade passa a contar so' identidades COM CPF. Nao funde ninguem |
| T-427 Presidencia e Mesa no Senado (F-31 / ADR-049) | OK | 05/09/2026: fecha a L-30 no mesmo dia em que ela abriu. A medicao dela estava certa (zero presidencias em 7.226 vinculos) e a conclusao errada — o dado esta' em `/senador/{cod}/cargos`, outra rota da MESMA API, que traz junto a Mesa Diretora. Procurei a Mesa por rotas plausiveis e nao pelo caminho que a API ja' usava para todo o resto. A primeira versao do mart combinou `papel_principal` com `em_curso` e publicou CINCO presidentes simultaneos na CDH; separado em `papel_atual`. Conferido pela invariante de uma presidencia por colegiado: 18 permanentes e a Mesa, zero duplicados |
| T-426 Ficha que mudou de endereco (F-30) | OK | 05/09/2026: 7 candidaturas com dois enderecos porque o slug vem do nome de urna e a pessoa corrigiu a grafia no TSE. O endereco velho continuava no ar servindo o nome errado. Vira encaminhamento com canonical e noindex, e nao e' apagado: 404 quebraria link ja' compartilhado |
| T-425 Relatorio analitico: coleta e geracao no repo (F-28) | OK | 05/09/2026: a primeira edicao vivia num diretorio temporario. Trazida para `scripts/dados_relatorio.py` + `scripts/gerar_relatorio.py`, a revisao encontrou seis numeros que tinham envelhecido no texto fixo: 'a UNICA candidatura acima de R$ 1 bilhao' (sao duas), 72 trocas de cargo (73), 'R$ 122 bilhoes pagos' (nao bate com recorte nenhum — sao 97,2 bi a autores identificados), 20.162 candidaturas (20.851), 29 lacunas (30) e 'a ficha de senador nao traz comissoes' (traz, desde a F-29). Todos passaram a ser lidos do dado |
| T-424 Comissoes do Senado (F-29 / ADR-048) | OK | 05/09/2026: fecha a L-28, cuja PREMISSA estava errada — ela exigia CPF para exibir comissao de senador enquanto o bloco de atividade legislativa do Senado ja' estava no ar com a mesma identidade inferida, em 50 fichas. Sobreviveu meses porque ninguem comparou as duas telas. A fonte trouxe a armadilha do catalogo: ele so' lista colegiado EM ATIVIDADE, e os 292 que faltavam eram 21% dos vinculos e os de maior peso publico (CPMI do INSS, CPI da Pandemia). Sem rota que resolva o encerrado, o tipo saiu do NOME oficial por extenso, nunca da sigla; cobertura foi de 4.258 para 5.514. Conferido contra o Regimento Interno: CCJ 27=27, e nenhuma comissao acima do teto regimental |
...
| F-31 / [ADR-049](adr/ADR-049-cargos-de-comando-no-senado.md) | Presidencia e Mesa no Senado | ✅ **Entregue** — 804 cargos de comando e 61 vinculos de Mesa onde havia zero. `papel_atual` separado de `papel_principal`: sem isso a CDH aparecia com cinco presidentes. Conferido por uma presidencia por colegiado |
| F-30 | Ficha que mudou de endereco | ✅ **Entregue** — o endereco antigo vira encaminhamento em vez de servir a grafia que a pessoa corrigiu, e nao e' apagado para nao quebrar link compartilhado |
| F-29 / [ADR-048](adr/ADR-048-comissoes-do-senado.md) | Comissoes no Senado | ✅ **Entregue** — 51 fichas de 2026. Identidade inferida por nome + nascimento, com a ressalva escrita no topo do bloco. Presidencia, relatoria e Mesa Diretora nao existem na fonte (zero em 7.226 vinculos) e a tela diz que nao afirma quem presidiu — L-30. Conferido contra o Regimento Interno do Senado |
| F-28 | Relatorio analitico em PDF na home | ✅ **Entregue** — 27 paginas com a leitura agregada dos mesmos dados das fichas. O bloco so' aparece se o arquivo existir, e por isso o PDF esta' versionado: sem ele, uma publicacao pelo GitHub Actions perderia o bloco em silencio |
