...
| ADR-051 | CPF digitado no campo do nome do doador | O arquivo de prestação de contas trouxe um CPF válido onde vai o nome; a ingestão hasheia a coluna de CPF e não tinha como cobrir isso. O `dbt build` é etapa fatal e parou a atualização antes de publicar | Aceita |
| ADR-050 | Registro sem CPF não é prova de uma segunda pessoa | `id_pessoa` é o cpf_hash quando o ano publica CPF e a chave de nome quando não publica; a mesma pessoa fica com dois ids ao cruzar essa fronteira, e contar ids distintos lia isso como homonímia — 210 das 326 chaves "ambíguas" eram fronteira, não homônimo | Aceita |
